...
public abstract class AbstractSyslogEventDefinition implements EventDefinition {

    private EventMatcher matcher;

    public AbstractSyslogEventDefinition(String match) {
        this(match, false);
    }

    public AbstractSyslogEventDefinition(String match, boolean regex) {
        if (!regex) {
            matcher = new SyslogEventSubstringMatcher(match);
        } else {
            matcher = new SyslogEventRegexMatcher(match);
        }
    }
